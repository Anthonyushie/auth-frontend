...
export const btnSmallSecondary =
  btnSmall +
  " border border-[#d6d3d1] bg-white text-[#44403c] hover:border-[#ff751f] hover:text-[#ff751f]";

export const btnSmallDanger =
  btnSmall +
  " border border-[#fecaca] bg-white text-[#b91c1c] hover:bg-[#fef2f2] hover:border-[#fca5a5]";
...
export function Kicker({ children }: { children: ReactNode }) {
  return (
    <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#a8a29e]">
      {children}
    </p>
  );
}
  ...
  return (
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div className="min-w-0">
        ...
        {lede && (
          <p className="mt-2 max-w-xl text-[14.5px] leading-relaxed text-[#78716c]">
            {lede}
          </p>
        )}
      </div>
...
const badgeTones: Record<BadgeTone, string> = {
  ...
  accent:
    "border-[#ffd9bd] bg-[#fff4ec] text-[#c2570b]",
  dark: "border-[#1c1917] bg-[#1c1917] text-white",
  success:
...
export function Section({
  ...
  return (
    <section className="mt-10">
      <div className="flex items-end justify-between gap-4 border-b border-[#e7e5e4] pb-3">
        <div>
          <h2 className="text-[15px] font-bold tracking-[-0.01em] text-[#1c1917]">
            {title}
          </h2>
          {lede && <p className="mt-0.5 text-[13px] text-[#78716c]">{lede}</p>}
        </div>
...
export function EmptyState({
  ...
  return (
    <div className="border-y border-[#e7e5e4] py-12 text-left">
      <h3 className="text-[16px] font-bold tracking-tight text-[#1c1917]">
        {title}
      </h3>
      <p className="mt-1.5 max-w-md text-sm leading-relaxed text-[#78716c]">
        {body}
      </p>
...
export function FieldError({ children }: { children: ReactNode }) {
  return (
    <div
      role="alert"
      className="rounded-md border border-[#fecaca] bg-[#fef2f2] px-3.5 py-2.5 text-[13.5px] font-medium leading-relaxed text-[#b91c1c]"
    >
      {children}
    </div>
  );
...
export function Notice({ children }: { children: ReactNode }) {
  return (
    <div
      role="status"
      className="rounded-md border border-[#bbf7d0] bg-[#f0fdf4] px-3.5 py-2.5 text-[13.5px] font-medium text-[#15803d]"
    >
      {children}
    </div>
  );
...
export function BackLink({ href, children }: { href: string; children: ReactNode }) {
  return (
    <Link
      href={href}
      className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-[#78716c] transition-colors hover:text-[#ff751f]"
    >
